...
Last updated: 2026-05-03 by Claude (Sprint 90 closeout — deferred-items cleanup: MVP voter calibration activated, Opportunity Uplift UI surfaced, Cloudflare deferrals refined)
...
| Target merge | TBD |
| Sprint shape | TBD |
| Branch | `master` until Sprint 91 kickoff |
| Worker policy | No active sprint; set at kickoff |

**Production status:** CourtVue Labs is publicly live at `https://courtvue.app` (Vercel) + `https://api.courtvue.app` (Hetzner CPX11, `ubuntu@5.78.114.15`). Sprint 90 wired the MVP Award Case voter calibration through end-to-end (live LOO-CV per request, cached 24h, surfaced as `calibration_metadata` on the MVP race response and `runtime_calibration` on the methodology registry's `mvp` domain) and surfaced Sprint 79's Opportunity Uplift evidence on the frontend (full-width `UpliftEvidenceCard` + compact row hint + methodology drawer subsection). 513 backend tests (was 509, +4 new), 0 lint errors. 2 genuine deferrals remain: R2 backup lifecycle (Cloudflare UI, ~5 min) and MVP voter calibration cohort expansion (data-blocked).
