...
import { useWarehouseSeasonHealth } from "@/hooks/usePlayerStats";
import {
  queueSeasonBackfill,
  queueCurrentSeason,
  runNextWarehouseJob,
} from "@/lib/api";
import type { SourceRunResponse } from "@/lib/types";
...
export default function WarehousePipelinePanel({ season }: Props) {
  ...
  async function handleAction(fn: () => Promise<{ queued?: number; status?: string }>, label: string) {
    setBusy(true);
    setActionMsg(null);
    setActionErr(null);
    try {
