...
import { usePlayerTracking } from "@/hooks/usePlayerStats";
import type { PlayerTrackingRow } from "@/lib/types";
...
const COLUMNS_BY_FAMILY: Record<
  string,
  Array<{ key: keyof PlayerTrackingRow; label: string; format?: "int" | "float1" }>
> = {
...
function TrackingTable({
  ...
  rows: PlayerTrackingRow[];
  columns: Array<{ key: keyof PlayerTrackingRow; label: string; format?: "int" | "float1" }>;
}) {
  const thCls =
...
const FAMILY_PREF_KEY = "cv-tracking-family";

export default function PlayerTrackingPanel({ playerId, season, isPlayoff = false }: Props) {
  const { data, isLoading, error } = usePlayerTracking(playerId, season, isPlayoff);
  ...
  if (error) {
    return (
      <div className="bip-panel rounded-[1.8rem] p-6 text-sm text-[var(--muted-strong)]">
        Player tracking data unavailable for {season}.
      </div>
    );
  }
  ...
  if (presentFamilies.length === 0) {
    return (
      <div className="bip-panel rounded-[1.8rem] p-6 text-sm text-[var(--muted-strong)]">
        No player-tracking data available for {season}.
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <section className="bip-panel-strong rounded-[2rem] p-6">
        <p className="bip-kicker">Player Tracking</p>
        <h2
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--muted-strong)]">
          Official NBA player-tracking dashboards for {season}. Powered by{" "}
