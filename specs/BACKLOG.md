...
The original SSH key on `5.78.114.15` was rejected last sprint. Rather than fight the VNC console for a root password reset (which has its own pitfalls — keyboard layout, paste echo, reboot timing), use **Hetzner rescue mode** to inject a key cleanly.

On Vivek's Mac, copy the public key:
```bash
cat ~/.ssh/id_ed25519.pub | pbcopy
# (If it doesn't exist: ssh-keygen -t ed25519 -C "vivtalla@gmail.com" — accept defaults)
```

In the Hetzner Cloud UI (https://console.hetzner.cloud → bip-prod server):
1. Left sidebar → **Rescue**
2. Operating system: `linux64`
3. **SSH keys**: paste the contents of `id_ed25519.pub`. If never added before, click "Add SSH key", name it `vivek-macbook`.
4. Click **Enable rescue & power cycle**. Wait ~30 seconds.

From the Mac:
```bash
ssh root@5.78.114.15
# Works because rescue mode trusts the SSH key just added.
mkdir -p /mnt/home/ubuntu/.ssh
cat /root/.ssh/authorized_keys >> /mnt/home/ubuntu/.ssh/authorized_keys
chmod 700 /mnt/home/ubuntu/.ssh
chmod 600 /mnt/home/ubuntu/.ssh/authorized_keys
chown -R 1000:1000 /mnt/home/ubuntu/.ssh   # 1000 = ubuntu user UID on Ubuntu Cloud images
exit
```

Back in Hetzner UI → **Rescue** → **Disable rescue** → **Power cycle**. Wait 30s. Verify from the Mac:
```bash
ssh ubuntu@5.78.114.15 'echo OK'   # Should print: OK
```

#### Phase 1 — Open firewall

Hetzner Cloud UI → **Firewalls** → existing CX22 firewall → add inbound TCP 80 and TCP 443 from `0.0.0.0/0, ::/0`. Keep port 5432 locked to laptop IP only.

#### Phase 2 — Register `courtvue.app` and configure Cloudflare DNS

Cloudflare dashboard → Domain Registration → register `courtvue.app` (~$12/yr). After purchase, in the new zone → DNS → Records, add:

| Type | Name | Target | Proxy |
|------|------|--------|-------|
| A | `api` | `5.78.114.15` | **Proxied (orange)** — public mode |
| CNAME | `@` | `cname.vercel-dns.com` | Proxied (orange) |
| CNAME | `www` | `cname.vercel-dns.com` | Proxied (orange) |

Verify: `dig +short api.courtvue.app` should resolve to a Cloudflare IP (orange-cloud) within ~1 min.

#### Phase 3 — Deploy FastAPI on the VM
...
sudo -u postgres psql -d bip -c "VACUUM FULL;"   # reclaim Sprint 81 play_by_play space

sudo systemctl start bip-api && sudo systemctl reload caddy
sudo journalctl -u caddy -f    # watch for "certificate obtained successfully", Ctrl+C
curl -sf https://api.courtvue.app/api/health    # 200 OK, no credentials
```

Optionally install Playwright for the PST scraper:
```bash
sudo bash /home/ubuntu/bip/infra/playwright-install.sh
```

#### Phase 4 — Deploy Next.js on Vercel

vercel.com/new → Import the GitHub repo. Settings:
...
Click Deploy. After build, add custom domains in **Project → Settings → Domains**: `courtvue.app` and `www.courtvue.app`.

#### Phase 5 — Cloudflare WAF + cache rules

Free tier — 5 custom rules + 1 rate limit + 5 cache rules. Configure per `infra/README.md`:

- **Security → WAF → Custom rules:** block `(http.user_agent eq "") or (http.user_agent contains "zgrab") or (http.user_agent contains "masscan")` → Block
- **Security → WAF → Rate limiting rules:** path `*api.courtvue.app/*`, threshold 100 req/10min per IP, action Managed Challenge
- **Caching → Cache Rules:** TTLs aligned with nightly cron — 30 min playoffs, 1 hr standings, 6 hr leaderboards, 12 hr splits, 5 min default

#### Phase 6 — Smoke test

From a phone (different IP than the laptop): `https://courtvue.app` should load fully, player profiles populate splits/play-types panels, no auth prompt.

If anything 502s on the VM:
```bash
sudo journalctl -u bip-api -n 100   # backend errors
sudo journalctl -u caddy -n 50      # proxy errors
sudo systemctl status bip-api caddy
```

**Total wall-clock time: ~30-45 min, almost entirely web UI clicks.**
