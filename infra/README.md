...
|------|-------------------------|--------------|-----|
| 1 | `hostname=api.courtvue.app AND URI matches "^/api/playoffs/(today\|bracket\|last-night-pulse)"` | **Bypass cache** | Powers home-page surfaces that must reflect game completion within minutes — the LiveTicker (polled every 60s on `/today`), the SeriesTrackerStrip + bracket views (polled every 5 min on `/bracket`), and the LastNightPulse rail (polled every 5 min on `/last-night-pulse`, Sprint 96). Free-tier 2hr Edge TTL minimum made "live" scores up to 2 hours stale, so these endpoints bypass the edge entirely. Origin load is bounded by per-tab SWR poll cadence and Cloudflare's 100 req/10 min/IP rate-limit rule, so the 2-worker gunicorn is fine without spike protection here. Sprint 91 added `/today`; Sprint 96 expanded the regex to cover `/bracket` + `/last-night-pulse` after Vivek surfaced 2-hour-stale series records on the home page. |
| 2 | `hostname=api.courtvue.app AND URI starts_with /api/playoffs/` | 2 hours | Series detail + per-series player logs + leaders + simulator. Rule 1 catches the live-fresh endpoints first (priority order matters). Refreshes post-game; 2hr is fine for everything else. |
