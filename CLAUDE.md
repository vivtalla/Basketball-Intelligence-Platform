...
- **Secrets:** `~/.bip-env` on laptop exports `DATABASE_URL=postgresql://bip:...@5.78.114.15:5432/bip`. `/etc/bip/env` on VM (`chmod 600`) holds `PGUSER`, `PGPASSWORD`, `DATABASE_URL`, and R2 credential placeholders.
- **Backup automation:** `infra/bip-backup.sh` (pg_dump → gzip → R2 stream), `infra/bip-backup-prune.sh` (7 daily / 4 weekly / 3 monthly retention), `infra/bip-backup-verify.sh` (weekly restore drill to `bip_restore_test`). R2 credentials to be filled in at `5.78.114.15:/etc/bip/env` after Cloudflare R2 bucket creation.
- **Pending post-sprint:** (1) Hetzner Cloud Firewall — lock port 5432 to laptop IP `97.115.178.47` only. (2) Cloudflare R2 bucket + API tokens — update `/etc/bip/env` R2 placeholders. (3) Disable laptop cron. Runbook: `specs/db-hosting.md`.
