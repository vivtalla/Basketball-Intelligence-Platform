...
### Sprint 98 — Data Foundation Hardening

- **Staff-engineering review pass on the data foundation.** Four parallel streams on a single branch (`feature/sprint-98-data-foundation`), 11 commits, 46 files changed (+3443 / -61). 628 backend tests (was 594, +34 new). `npm run build` + `npm run lint` clean. Pre-merge to master at `c5b6c9a`.
- **Stream A — Sync reliability & alerting:** new `services/sync_freshness.py` generalizes Sprint 97's playoff-backfill marker pattern across every periodic sync (22 known entities with cadence tiers: 15min post-game, 1440min daily, 10080min weekly). `read_all_syncs()` powers an expanded `/api/health/sync-status` response with a `syncs` map + `cache_db` size snapshot — legacy `playoff_backfill_last_24h` key preserved for back-compat. New `services/regular_season_gap_detector.py` mirrors the playoff backfill for regular-season games via the CDN season schedule. New `utils/logging_setup.py` configures `structlog` with JSON in production + per-cron `BIP_RUN_ID` + per-request `X-Request-ID` middleware. New `utils/sentry_init.py` (free-tier Sentry, 5K/mo) wired through `configure_logging()`. `daily_sync.sh` generates a UUID RUN_ID at entry, dumps env to `/var/log/bip-cron-env/` for one-week root-cause study, calls `record_sync()` at every major heredoc. `infra/README.md` documents UptimeRobot setup with a keyword monitor on `"stale": true`.
- **Stream B — Schema integrity (surgical):** three additive Alembic revisions. 0025 adds FK constraints on `PlayoffSeries.parent_top/bottom_series_id` (ON DELETE SET NULL) + unique index on `series_id` to enable the FK target, defensively nulls orphans pre-FK. 0026 adds `created_at`/`updated_at` to `AwardCaseCandidate` + `RoleExpansionObservation`, backfilled from existing audit columns. 0027 creates `playoff_series_win_truth` view computing wins from `game_logs` at query time. Denormalized `top_wins`/`bottom_wins` kept; view powers a new `services/playoff_drift_detector.py` + `/api/admin/playoff-series-drift` admin-key gated endpoint (empty list = healthy). New `services/external_metric_staleness.py` exposes per-metric `as_of` from `SeasonStat.external_metrics_meta` JSON; `LeaderboardEntry.metric_as_of` populated when stat is external.
- **Stream C — API surface hardening:** `_block_live_fetch_if_user_mode` extended from 8 → 24 nba_client wrappers. 0 unguarded stats.nba.com endpoints; 8 CDN-only public endpoints (boxscore, PBP, scoreboard, schedule, injuries) intentionally unguarded — designed-public, fast, rate-limit-free. New `scripts/audit_nba_client_guards.py` classifies every network-calling function into guarded / private-helper / CDN-only-public / concern (final: 0 concerns). 8 admin/sync mutation endpoints newly admin-key gated (players sync, injuries sync/resolve/delete, shotchart refresh ops). slowapi rate limits added to `/api/query/ask` 10/min, `/api/trade/{validate,impact}` 30/20 min. The plan flagged ~43 mutations; the rest are user-facing computational POSTs (snapshots, lineup builder, custom metrics) protected by rate limits instead.
- **Stream D — Test coverage & CI:** shared `tests/conftest.py` with `test_db_session` / `client` / `admin_client` / `seed_basic` fixtures (view bootstrapped via raw SQL so admin drift tests work without the full Alembic chain). 12 new smoke tests across health endpoints + 11 router GET surfaces + request-ID propagation. New `.github/workflows/ci.yml` runs pytest blocking + ruff informational (grace period for 73 baseline issues) + frontend lint informational + frontend build blocking on every PR. `backend/ruff.toml` + `backend/requirements-dev.txt` (pinned dev deps: ruff, pytest, httpx).
- **Doc:** new `specs/architecture-flows.html` — single self-contained HTML, 20 nodes across 9 layers, 21 flows × 154 steps. Layered SVG diagram + clickable flow picker + step panel with prev/next navigation. Inline JSON catalog.
- **Workflow lessons:** Local venv parity matters — mid-QA discovered `slowapi` not installed locally (rate limit silently no-op'd until installed), `httpx` missing (TestClient broke), local DB at 0023 (Sprint 88) — not even 0024 applied. Worth a `make verify` that confirms local env matches deploy env. The plan's frontend `metric_as_of` chip promise on `MvpRacePanel`/`ExternalMetricsPanel` was overly optimistic; both components don't currently receive `external_metrics_meta` from their data sources. Plan-time grep for the components' data sources would have surfaced it; deferred to a future sprint that threads `metric_staleness` through `MvpAdvancedProfile`/`SeasonStats`.
- **Deferred:** D3 deep service tests for 10 high-risk untested services (Why: different domain — incremental growth as services are touched); frontend `metric_as_of` chip (Why: different domain — needs response-shape threading); cron env-propagation root cause (Why: blocked on data — one-week capture in flight); Cloudflare cache bypass on `/api/admin/*` (Why: different domain — Cloudflare UI step); ruff blocking in CI (Why: 73-issue baseline cleanup first). Closeout: `specs/sprint-98-closeout.md`.
...
*Sprint 96 and earlier moved to `specs/sprint-history.md`.*
