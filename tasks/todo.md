# Sprint 38 Closeout Plan

## Scope
- Document what actually shipped in Sprint 38.
- Refresh the backlog so shipped items become follow-ons or are removed.
- Reset `AGENTS.md` for the next sprint kickoff.
- Update `CLAUDE.md` recent-sprint notes and `specs/sprint-history.md`.
- Verify the repo is clean, then commit, merge, and push Sprint 38 to `master`.

## Checklist
- [x] Write `specs/sprint-38-closeout.md`
- [x] Refresh `specs/BACKLOG.md`
- [x] Update `specs/sprint-history.md`
- [x] Update `CLAUDE.md`
- [x] Reset `AGENTS.md` for Sprint 39 kickoff
- [x] Verify lint/build/test status
- [ ] Commit Sprint 38 changes
- [ ] Merge to `master`
- [ ] Push `master` to `origin/master`

## Review
- Closeout package should reflect the shipped data foundation, shot-lab follow-through, and 3D visualizer work.
- Preserve the backlog item for forward-compatible completeness and the new 3D/play-visualization follow-ons.
- Sprint 38 closeout docs now mirror the shipped code and the sprint is ready to be packaged into the merge commit.
